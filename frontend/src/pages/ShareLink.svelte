<script lang="ts">
  ...
  let resources: Array<{
    id: string; title: string; filename: string; file_size: number;
    content_type: string; resource_type: string; views: number; created_at: string;
  }> = [];
  let playlists: Array<{name: string; display_name: string; playlist_type: string}> = [];
  let selectedPlaylist = '';

  onMount(() => loadResources());

  async function loadResources(playlistName?: string) {
    loading = true;
    error = null;
    try {
      const result = await getShareLinkResources(id, password, playlistName);
  ...
  function onPlaylistChange() {
    if (selectedPlaylist) {
      loadResources(selectedPlaylist);
    } else {
      loadResources();
    }
  }
  ...
  {:else}
    <div class="rounded-lg border border-gray-200 bg-white p-6 mb-4">
      <div class="flex items-center justify-between">
        ...
        {#if targetType === 'category' && playlists.length > 0}
          <div class="flex items-center gap-2">
            <label for="playlist-filter" class="text-sm text-gray-600">Playlist:</label>
            <select id="playlist-filter" bind:value={selectedPlaylist} on:change={onPlaylistChange} class="text-sm border border-gray-300 rounded-md px-2 py-1">
              <option value="">All files</option>
              {#each playlists as pl}
                <option value={pl.name}>{pl.display_name || pl.name}</option>
              {/each}
            </select>
          </div>
        {/if}
          ...
          <tbody>
            {#each resources as res}
              <tr class="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td class="py-2 px-4">
                  <a href={getDownloadUrl(res)} target="_blank"
